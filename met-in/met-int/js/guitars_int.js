...
function setupAux() {
  ...
  textAlign(LEFT);
  textSize(16);
  text("Click guitar strings for play/pause", 210, 2642);
  textSize(18);
  text("Stratocaster", 210, 2860);
  text("Les Paul", 210, 3150);
  image(fenderStrat, 200, 2680);
  image(gibsonLP, 195, 2960);
  ...
  textSize(16);
  textStyle(NORMAL);
  text("Date of Manufacture", 130, topMargin + 170);
  textSize(20);
...
the corresponding row.*/
  ...
  for (var t = 1940; t <= 2000; t += 10) {
    ...
    text(tvalues, tvalue, 425 + guitarLocations.length * 40);
    if (filterSelection === "All") {
      let textY = (395 + 425 + guitarLocations.length * 40) / 2;
      fill(60);
      text(tvalues, tvalue, textY - 13);
    }
  }
  textSize(20);
  fill(255);
...
function mouseClicked() {
  ...
  if ((mouseX >= 260 && mouseX <= 580) && (mouseY >= 2735 && mouseY <= 2765)) {
    togglePlay("fender"); 
    // fill(0);
    // rect(798, topMargin + 2448, 202, 202);
    stroke(162, 224, 184);
  ...
  else if ((mouseX >= 275 && mouseX <= 595) && (mouseY >= 3025 && mouseY <= 3055)) {
    togglePlay("gibson");
    // fill(0);
    // rect(798, topMargin + 2748, 202, 202);
    stroke(183, 132, 67);
